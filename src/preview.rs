...
/// The prompt drops the guard before echoing the selected key, matching `rep`'s
/// cooked-mode echo behavior while still restoring the terminal on early exits.
struct RawModeGuard;
...
fn display_parent(path: &Path) -> Option<String> {
...
fn print_inline_diff(old_path: &Path, new_path: &Path) {
    ...
    // Drop a bare "." or leading "./" so cwd-rooted plans don't render the
    // redundant prefix: "./foo.txt" renders as "foo.txt".
    let parent = display_parent(old_path);
...
impl PreviewPatcher {
    ...
    /// prompting. `q` stops prompting and returns the accepted entries so far.
    pub(crate) fn prompt_plan(&mut self, plan: &[PlanEntry]) -> Result<Vec<PlanEntry>> {
        ...
        Ok(accepted_entries(plan, &decisions))
    }
}

fn accepted_entries(plan: &[PlanEntry], decisions: &[(usize, bool)]) -> Vec<PlanEntry> {
    plan.iter()
        .enumerate()
...
mod tests {
    ...
    use super::accepted_entries;
    use super::common_prefix_suffix;
    use super::decide;
    use super::display_parent;
    use crate::rename::PlanEntry;
    ...
    /// The accepted-subset filter yields the entries whose index appears with
    ...
    fn accepted_subset_filter_matches_decision_stack() {
        ...
        let accepted = accepted_entries(&plan, &decisions);
    ...
    #[test]
    fn accepted_entries_supports_quit_after_partial_accepts() {
        let plan = [
            PlanEntry {
                old: PathBuf::from("a"),
                new: PathBuf::from("A"),
    ...
    #[test]
    fn display_parent_drops_cwd_prefix() {
        assert_eq!(display_parent(PathBuf::from("LICENSE").as_path()), None);
        assert_eq!(display_parent(PathBuf::from("./LICENSE").as_path()), None);
        assert_eq!(
            display_parent(PathBuf::from("./src/main.rs").as_path()),
            Some("src".into())
        );
    }
